...
type State = {
  ...
  customName1: string;
  customPct1: number;
  customWeight1: number;
  customName2: string;
  customPct2: number;
  customWeight2: number;
  customName3: string;
  customPct3: number;
  customWeight3: number;
  target: number;
  loaves: number;
  count: number;
  ball: number;
...
const baseDefaults: State = {
  ...
  bpSugar: 0,
  customCount: 0,
  customName1: 'Ingredient 1',
  customPct1: 0,
  customWeight1: 0,
  customName2: 'Ingredient 2',
  customPct2: 0,
  customWeight2: 0,
  customName3: 'Ingredient 3',
  customPct3: 0,
  customWeight3: 0,
  target: 1500,
  loaves: 2,
...
function readText(params: URLSearchParams, key: keyof State, fallback: string) {
  const value = params.get(key);
  return value && value.length <= 40 ? value : fallback;
}
...
function Field(props: { label: string; value: number; set: (next: number) => void; unit: string; step?: number; min?: number; helper?: string; error?: string }) {
  ...
  return (
    <label className="rounded-2xl border bg-white p-4">
      ...
      <span className="mt-2 flex overflow-hidden rounded-xl border">
        ...
        <input className="min-h-12 w-full px-3" inputMode="decimal" type="number" value={props.value} min={min} step={step} onChange={(event) => update(Number(event.target.value))} aria-label={props.label} aria-invalid={Boolean(props.error)} />
      ...
      {props.error ? <span className="mt-1 block text-xs text-red-700" role="alert">{props.error}</span> : null}
    </label>
  );
}

function TextField(props: { label: string; value: string; set: (next: string) => void }) {
  return <label className="rounded-2xl border bg-white p-4"><span className="text-sm font-medium">{props.label}</span><input className="mt-2 min-h-12 w-full rounded-xl border px-3" value={props.value} maxLength={40} onChange={(event) => props.set(event.target.value)} aria-label={props.label} /></label>;
}
...
function WarningList({ items, slug }: { items: Warn[]; slug: string }) {
  useEffect(() => { items.forEach((warning) => trackCalculatorEvent('warning_shown', slug, { warning_code: warning.code })); }, [items, slug]);
...
function HydrationBreakdown({ result, unit }: { result: Record<string, unknown>; unit: WeightUnit }) {
  const totalFlour = typeof result.totalFlourGrams === 'number' ? result.totalFlourGrams : undefined;
  const totalWater = typeof result.totalWaterGrams === 'number' ? result.totalWaterGrams : undefined;
  if (!totalFlour || !totalWater) return null;
  const starterSplit = result.starterSplit as { flourGrams?: number; waterGrams?: number } | undefined;
  const starterFlour = typeof result.starterFlourGrams === 'number' ? result.starterFlourGrams : (starterSplit?.flourGrams ?? 0);
  const starterWater = typeof result.starterWaterGrams === 'number' ? result.starterWaterGrams : (starterSplit?.waterGrams ?? 0);
  return (
    <div className="overflow-hidden rounded-2xl border bg-white">
      <h2 className="bg-stone-50 px-4 py-3 font-semibold">Hydration breakdown</h2>
      <table className="w-full text-left text-sm"><thead><tr><th className="p-3">Component</th><th className="p-3 text-right">Flour</th><th className="p-3 text-right">Water</th><th className="p-3 text-right">Total</th></tr></thead><tbody>
        <tr className="border-t"><td className="p-3 font-medium">Base dough</td><td className="p-3 text-right">{formatWeight(totalFlour - starterFlour, unit)}</td><td className="p-3 text-right">{formatWeight(totalWater - starterWater, unit)}</td><td className="p-3 text-right">{formatWeight(totalFlour + totalWater - starterFlour - starterWater, unit)}</td></tr>
        <tr className="border-t"><td className="p-3 font-medium">Starter</td><td className="p-3 text-right">{formatWeight(starterFlour, unit)}</td><td className="p-3 text-right">{formatWeight(starterWater, unit)}</td><td className="p-3 text-right">{formatWeight(starterFlour + starterWater, unit)}</td></tr>
        <tr className="border-t"><td className="p-3 font-medium">Total</td><td className="p-3 text-right">{formatWeight(totalFlour, unit)}</td><td className="p-3 text-right">{formatWeight(totalWater, unit)}</td><td className="p-3 text-right">{formatWeight(totalFlour + totalWater, unit)}</td></tr>
      </tbody></table>
    </div>
  );
}
...
function useCalculatorState(slug: string, defaultInputs?: DefaultInputs) {
  ...
  const [state, setState] = useState<State>(() => ({ ...defaults,
    bakerMode: readBakerMode(params, defaults.bakerMode), flour: readNumber(params, 'flour', defaults.flour), hyd: readNumber(params, 'hyd', defaults.hyd), starter: readNumber(params, 'starter', defaults.starter), salt: readNumber(params, 'salt', defaults.salt), water: readNumber(params, 'water', defaults.water), sh: readNumber(params, 'sh', defaults.sh), saltg: readNumber(params, 'saltg', defaults.saltg), bpWater: readNumber(params, 'bpWater', defaults.bpWater), bpStarter: readNumber(params, 'bpStarter', defaults.bpStarter), bpSalt: readNumber(params, 'bpSalt', defaults.bpSalt), bpOil: readNumber(params, 'bpOil', defaults.bpOil), bpSugar: readNumber(params, 'bpSugar', defaults.bpSugar), customCount: readNumber(params, 'customCount', defaults.customCount), customName1: readText(params, 'customName1', defaults.customName1), customPct1: readNumber(params, 'customPct1', defaults.customPct1), customWeight1: readNumber(params, 'customWeight1', defaults.customWeight1), customName2: readText(params, 'customName2', defaults.customName2), customPct2: readNumber(params, 'customPct2', defaults.customPct2), customWeight2: readNumber(params, 'customWeight2', defaults.customWeight2), customName3: readText(params, 'customName3', defaults.customName3), customPct3: readNumber(params, 'customPct3', defaults.customPct3), customWeight3: readNumber(params, 'customWeight3', defaults.customWeight3), target: readNumber(params, 'target', defaults.target), loaves: readNumber(params, 'loaves', defaults.loaves), count: readNumber(params, 'count', defaults.count), ball: readNumber(params, 'ball', defaults.ball), oil: readNumber(params, 'oil', defaults.oil), sugar: readNumber(params, 'sugar', defaults.sugar), yeast: readNumber(params, 'yeast', defaults.yeast), seed: readNumber(params, 'seed', defaults.seed), flourpart: readNumber(params, 'flourpart', defaults.flourpart), waterpart: readNumber(params, 'waterpart', defaults.waterpart), extra: readNumber(params, 'extra', defaults.extra), mode: params.get('mode') === 'flour' || params.get('mode') === 'target' ? (params.get('mode') as 'target' | 'flour') : defaults.mode, lev: params.get('lev') === 'sourdough' || params.get('lev') === 'yeast' ? (params.get('lev') as 'yeast' | 'sourdough') : defaults.lev, unit: readUnit(params, defaults.unit) }));
  useEffect(() => { trackCalculatorEvent('calculator_view', slug); }, [slug]);
  useEffect(() => { const url = new URL(location.href); Object.entries(state).forEach(([key, value]) => url.searchParams.set(key, String(value))); history.replaceState(null, '', `${url.pathname}?${url.searchParams.toString()}`); trackCalculatorEvent('calculate_click', slug); }, [state, slug]);
  const set = (key: keyof State, value: number | string) => setState((current) => ({ ...current, [key]: value }));
  const setUnit = (value: string) => setState((current) => { const next = value === 'oz' || value === 'lb' || value === 'g' ? value : current.unit; trackCalculatorEvent('unit_toggle', slug, { from: current.unit, to: next }); return { ...current, unit: next }; });
  return { state, set, setUnit };
}

function customIngredients(state: State) {
  const rows = [
    { name: state.customName1, percentage: state.customPct1, weightGrams: state.customWeight1 },
    { name: state.customName2, percentage: state.customPct2, weightGrams: state.customWeight2 },
    { name: state.customName3, percentage: state.customPct3, weightGrams: state.customWeight3 }
  ];
  return rows.slice(0, Math.max(0, Math.min(3, Math.floor(state.customCount))));
}
...
export default function Calculator({ slug, calculatorType, defaultInputs }: { slug: string; calculatorType: CalculatorType; defaultInputs?: DefaultInputs }) {
  const { state, set, setUnit } = useCalculatorState(slug, defaultInputs);
  const result = useMemo(() => {
    if (calculatorType === 'bakers-percentage') return state.bakerMode === 'weights' ? calculateBakersPercentagesFromWeights({ flourWeightGrams: state.flour, waterWeightGrams: state.bpWater, starterWeightGrams: state.bpStarter, saltWeightGrams: state.bpSalt, oilWeightGrams: state.bpOil, sugarWeightGrams: state.bpSugar, customIngredients: customIngredients(state) }) : calculateBakersPercentage({ flourWeightGrams: state.flour, hydrationPct: state.hyd, starterPct: state.starter, saltPct: state.salt, oilPct: state.oil, sugarPct: state.sugar, customIngredients: customIngredients(state) });
  ...
  return (
    <section className="rounded-3xl border border-amber-100 bg-amber-50 p-4 shadow-sm sm:p-6">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        {calculatorType === 'dough-scaling' ? <><SelectField label="Calculation mode" value={state.mode} set={(value) => set('mode', value)} options={[{ value: 'target', label: 'Target dough weight' }, { value: 'flour', label: 'Known flour weight' }]} />{state.mode === 'target' ? <Field label="Target dough" value={state.target} set={(value) => set('target', value)} unit="g" step={50} /> : <Field label="Flour weight" value={state.flour} set={(value) => set('flour', value)} unit="g" step={10} />}<Field label="Loaf count" value={state.loaves} set={(value) => set('loaves', value)} unit="loaves" /><Field label="Hydration" value={state.hyd} set={(value) => set('hyd', value)} unit="%" /><Field label="Starter" value={state.starter} set={(value) => set('starter', value)} unit="%" /><Field label="Starter hydration" value={state.sh} set={(value) => set('sh', value)} unit="%" /><Field label="Salt" value={state.salt} set={(value) => set('salt', value)} unit="%" step={0.1} /><Field label="Oil" value={state.oil} set={(value) => set('oil', value)} unit="%" step={0.5} /><Field label="Sugar" value={state.sugar} set={(value) => set('sugar', value)} unit="%" step={0.5} /><Field label="Yeast" value={state.yeast} set={(value) => set('yeast', value)} unit="%" step={0.1} /></> : null}
      </div>
      {calculatorType === 'bakers-percentage' ? <div className="no-print mt-4 rounded-2xl border bg-white p-4"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">Custom ingredient rows</h2><div className="flex gap-2"><button className="rounded-full border px-3 py-1 text-sm" type="button" onClick={() => set('customCount', Math.min(3, state.customCount + 1))}>Add row</button><button className="rounded-full border px-3 py-1 text-sm" type="button" onClick={() => set('customCount', Math.max(0, state.customCount - 1))}>Remove row</button></div></div><div className="mt-4 grid gap-4 md:grid-cols-3">{[1, 2, 3].slice(0, state.customCount).map((index) => <div className="grid gap-3 rounded-xl bg-stone-50 p-3" key={index}><TextField label={`Custom ${index} name`} value={state[`customName${index}` as keyof State] as string} set={(value) => set(`customName${index}` as keyof State, value)} /><Field label={state.bakerMode === 'weights' ? `Custom ${index} weight` : `Custom ${index} percentage`} value={state[state.bakerMode === 'weights' ? `customWeight${index}` as keyof State : `customPct${index}` as keyof State] as number} set={(value) => set(state.bakerMode === 'weights' ? `customWeight${index}` as keyof State : `customPct${index}` as keyof State, value)} unit={state.bakerMode === 'weights' ? 'g' : '%'} step={state.bakerMode === 'weights' ? 5 : 0.5} /></div>)}</div></div> : null}
      {calculatorType === 'starter-feeding' ? <div className="mt-4"><Presets set={set} /></div> : null}
      <div className="mt-6 space-y-5 print-card">
        ...
        <HydrationBreakdown result={result as Record<string, unknown>} unit={state.unit} />
        ...
        <p className="text-xs text-stone-500">Input summary: flour {round(state.flour)}g, hydration {round(state.hyd)}%, starter {round(state.starter)}%, salt {round(state.salt)}%.</p>
